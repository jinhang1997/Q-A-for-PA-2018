...
1. 80386手册PDF第`411`页，关于`XOR异或指令`的指令码`32`和`33`形式的描述`把源和目的操作数写反了`。我们说手册上是`Intel格式`，目的操作数在前，因此opcode为32和33的描述表述写反了。感谢20327同学。修改如下：
```
...
# PA2
6.对于指令xchg:框架里并没有预先给我们定义make_EHelper(xchg)（可能是作者遗漏了）,需要自己定义（应该是属于data-mov一类的）。
